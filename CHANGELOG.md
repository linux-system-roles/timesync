...
- feat: Argument spec implementation for timesync role (#380)

### Other Changes

- ci: use commit hash for github action, add persist-credentials false [citest_skip] (#368)
- ci: refactor build_docs so that pandoc runs in isolated read-only job [citest_skip] (#369)
- ci: use exact match for systemroller instead of contains [citest_skip] (#370)
- ci(build_docs): fix pandoc container syntax [citest_skip] (#371)
- refactor: Relax collection constraints, gate vendored modules by python version, update ci versions [citest_skip] (#375)
- ci: do not run ci tests by default, require citest comment or label [citest_skip] (#376)
- ci: replace weekly_ci with periodic_ci, stagger schedules [citest_skip] (#377)
- ci: update status when action triggered by issue comment (#378)
...
- test: ensure fingerprint test works in check mode (#366)
